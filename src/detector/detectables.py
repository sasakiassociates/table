    ...
    def notify_observers(self):
        for observer in self.observers:
            if self.is_visible:
                ...
                observer.remove(self.type, self.uuid)

    def is_in_area(self, area):
        if self.center[0] > area[0] and self.center[0] < area[2] and self.center[1] > area[1] and self.center[1] < area[3]:
            return True
        else:
            return False
...
class Collection(DetectableObject):
    def __init__(self, marker_id, timer_):
        super().__init__(marker_id, timer_)
